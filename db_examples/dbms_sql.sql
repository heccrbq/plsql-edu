...
declare
    l_cur integer;
    l_sql varchar2(4000) := q'[select value(xt).getnumberval() from xmltable('1 to xs:integer(.)' passing :1) xt]';
    l_rowcount integer;
    -- define columns
...
begin
    ...
    dbms_sql.define_array(l_cur, 1, l_num_list, 0, 0); 
    
    l_rowcount := dbms_sql.execute(l_cur);
    
    while dbms_sql.fetch_rows(l_cur) > 0
    loop
        dbms_sql.column_value(l_cur, 1, l_num_list);
    end loop;
...
begin
    ...
    loop
        if l_collist(i).col_type = 2 then
            dbms_sql.define_column(l_cur, i, l_numval);
        else
            dbms_sql.define_column(l_cur, i, l_var2val, 50);
        end if;
    end loop;
